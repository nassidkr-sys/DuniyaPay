...
function NotifItem({ title, desc, time, isNew = false }: { title: string, desc: string, time: string, isNew?: boolean }) {
  return (
    <div style={{ background: 'var(--bg-white)', padding: '16px', borderRadius: '16px', border: '1px solid var(--border)', position: 'relative', display: 'flex', flexDirection: 'column', gap: '4px' }}>
      {isNew && (
